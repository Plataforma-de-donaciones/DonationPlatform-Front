...
function LocalidadBox({ onChange, eqZone, setEqZone }) {
  const [zones, setZones] = useState([]);
  const [selectedZone, setSelectedZone] = useState("");
  console.log("coso zona", eqZone);
  ...
  const handleZoneChange = (event) => {
    ...
    if (setEqZone && typeof setEqZone === 'function') {
      setEqZone(selectedValue);
    }
  };
