...
const ActionButton = styled.button`
  ...
  background-color: ${(props) =>
    props.secondary ? "#ccc" : "rgba(79,181,139, 1)"};
  color: #fff;
  border: none;
  border-radius: 4px;
  cursor: pointer;
  ...
  &:hover {
    background-color: ${(props) =>
      props.secondary ? "#ff0000" : "rgba(141, 202, 170, 1)"};
  }
`;
...
const EventListItem = ({ evento }) => {
  ...
  const handleUbicacion = () => {
    ...
    if (evento.zone === 1) {
      ...
      coordinates = { lat: -57.96667, lng: -31.38333 };
      ...
      coordinates = { lat: -34.09556, lng: -56.21417 };
    }
  ...
  const construirURLCompartir = (evento, redSocial) => {
    const textoEvento = encodeURIComponent(
      `Mira el evento: ${evento.event_name}, publicado en DonacionesUy. Haz clic en el link para visualizarlo. ¡Se parte de DonacionesUy, transformamos intenciones en impacto social!`
    );
    const urlEvento = encodeURIComponent(
      `https://donacionesuy.azurewebsites.net/listadoeventos`
    );
    ...
    switch (redSocial) {
      ...
      case "facebook":
        return `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
          urlEvento
        )}&quote=${encodeURIComponent(textoEvento)}`;
      case "instagram":
    ...
    return (
      <div style={{ cursor: "pointer" }} onClick={() => onClick(redSocial)}>
        ...
        <p style={{ textAlign: "center", margin: 0 }}>
          {redSocial.charAt(0).toUpperCase() + redSocial.slice(1)}
        </p>
      </div>
    );
  };
  return (
    <>
      <Card className="mb-3">
        <Card.Title className="text-center mt-3 card-titulo">
          {evento.event_name}
        </Card.Title>
        ...
        <Card.Body>
          <Row className="text-center">
            <Col>
              {evento.attachments != null && (
                <Image
                  src={evento.attachments}
                  alt="Evento"
                  onError={(e) => {
                    e.target.src =
                      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQwy_FB-xJcIjI4MkODy3cCyEM1zLACOC_LxpKdwSqPpI6IsMAU6inmenuTv9j8fgmfwHk&usqp=CAU";
                  }}
                />
              )}
              {evento.attachments == null
            ...
            <Col>
              <div>
                ...
                <span style={{ textAlign: "left" }}>
                  {" "}
                  {evento.formattedDateTime.formattedDate}
                </span>
              </div>
              <div>
                ...
                <span style={{ textAlign: "left" }}>
                  {" "}
                  {evento.formattedDateTime.formattedTime}
                </span>
              </div>
            </Col>
            ...
            <Col className="align-middle">
              <div>
                ...
                <span className="align-middle">
                  {" "}
                  {evento.formattedEndDate.formattedDate}
                </span>
              </div>
              <div>
                ...
                <span style={{ textAlign: "left" }}>
                  {" "}
                  {evento.formattedEndDate.formattedTime}
                </span>
              </div>
            </Col>
          </Row>
      ...
      <Modal show={mostrarCompartir} onHide={handleCloseCompartir} centered>
        ...
        <Modal.Body>
          <div style={{ display: "flex", justifyContent: "space-around" }}>
            <IconoRedSocial
              icono={FaTwitter}
              redSocial={RedesSociales.TWITTER}
              onClick={handleRedSocialClick}
            />
            <IconoRedSocial
              icono={FaFacebook}
              redSocial={RedesSociales.FACEBOOK}
              onClick={handleRedSocialClick}
            />
            <IconoRedSocial
              icono={FaWhatsapp}
              redSocial={RedesSociales.WHATSAPP}
              onClick={handleRedSocialClick}
            />
          </div>
        </Modal.Body>
